  ...
  <div :class="$q.screen.gt.xs ? 'q-ma-lg' : 'q-mt-lg'">
   ...
   <q-table
    ...
    <template v-slot:item="props">
        <q-intersection transition="scale" :key="props.row.id" style="min-height: 74px" v-if="sede !== null" class="q-pa-xs col-xs-12 col-sm-6 col-md-4 col-lg-3 grid-style-transition"
    ...
    lascats () {
      ...
      function red (items) {
        ...
        for (let j of Object.keys(items)) {
          let item = items[j]
          if (item.subCat && Object.keys(item.subCat).length) {
            itemchild = red(item.subCat)
            obj.push(...itemchild)
          }
          delete item.subCat
          obj.push(item)
      ...
      for (let i of categorias) {
        n = { ...i }
        if (i.subCat && Object.keys(i.subCat).length) {
          child = red(i.subCat)
          arr.push(...child)
        }
        delete n.subCat
  ...
  data () {
    return {
      ...
      columns: [{ name: 'photo', align: 'center', label: 'Foto', field: 'photo' },
        ...
        { name: 'stock', align: 'center', field: 'stock', label: 'Stock (und.)', sortable: true, sort: (a, b) => (parseInt(a[this.sede] || 0)) - (parseInt(b[this.sede]) || 0) },

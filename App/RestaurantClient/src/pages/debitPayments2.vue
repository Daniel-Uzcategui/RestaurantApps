 ...
 <q-item>
    ...
    <div >
        <div class="card-input"><label for="phonePassword" aria-label="Card CVV" >Clave telefonica</label>
             <q-input type="tel" v-model="valueFields.phonePassword"  title="clave telefonica" maxlength="4" data-card-field="" autocomplete="off"/>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
...
export default {
  ...
  methods: {
    ...
    encryptar (data) {
      ...
      cipher = this.CryptoJs.createCipheriv('aes-128-ecb', keypadding.substr(0, 16), '')
      let encryptedResult = cipher.update('782', 'ascii', 'hex')
      console.log('data en encriptar:', encryptedResult)
    ...
    decrypt (encrypted) {
      // let keypadding = this.keybankhash
      // let keypadding = this.keybankhash
      let keypadding = '1234567890123456'
      let key = keypadding.substr(0, 16)
      // const key = Buffer.from(keypadding.substr(0, 16), 'hex')
      console.log('data en deencriptar:', encrypted)
      console.log('key en deencriptar:', key)
      // console.log(this.base64)
      // encrypted = this.base64.b64decode(encrypted)
      // console.log(this.CryptoJs.Decipher)
      const decipher = this.CryptoJs.createDecipher('aes-128-ecb', key)
      decipher.setAutoPadding(true)
      let decryptedBytes = decipher.update(atob(encrypted))
      console.log('desencriptadoBytes:', decryptedBytes)
      let decrypted
      decrypted = new TextDecoder('utf-8').decode(decryptedBytes)
      return decrypted
    },
    hexStringToByte (str) {
      if (!str) {
    ...
    async payment () {
      ...
      let encodedEncryptedCVV = ''
      // let encodedEncryptedClave = ''
      ...
      // temp
      encodedEncryptedCVV = this.encryptar(this.encode_utf8(this.valueFields.cardCvv))
      console.log('encodedEncryptedCVV1:', encodedEncryptedCVV)
      typePasswordBank = this.decrypt(encodedEncryptedCVV)
      ...
      // proceso de pago
      encodedEncryptedCVV = this.encryptar(this.encode_utf8(this.valueFields.cardCvv))
      encodedEncryptedClave = this.encryptar(this.encode_utf8(this.valueFields.phonePassword))
      console.log('CVV:', this.valueFields.cardCvv)
      console.log('encodedEncryptedCVV:', encodedEncryptedCVV)
      console.log('phonePassword:', this.valueFields.phonePassword)
      console.log('encodedEncryptedClave:', encodedEncryptedClave)
      // let respuestaPay = this.paymentbank(encodedEncryptedCVV, encodedEncryptedClave)
      // console.log(respuestaPay)
      // log de transaccion */
    },
    async paymentbank (twofactorAuth, cvv) {
      let integratorId = 1

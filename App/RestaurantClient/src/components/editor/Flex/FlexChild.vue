<template>
  ...
  <q-input filled v-model="child['min-width']" dense label="Min Width (ex: '200px', '20em')" @input="emitChange">
    <template v-if="child && child['min-width'] && child['min-width'].length > 0" v-slot:append>
      <q-btn :icon="mdiClose" size="xs" flat dense @click="child['min-width'] = ''"/>
    </template>
  </q-input>

  <q-input filled v-model="child['min-height']" dense label="Min Height (ex: '300px', '25em')" @input="emitChange">
    <template v-if="child && child['min-height'] && child['min-height'].length > 0" v-slot:append>
      <q-btn :icon="mdiClose" size="xs" flat dense @click="child['min-height'] = ''"/>
    </template>
  </q-input>
  <q-input filled v-model="child['max-width']" dense label="Max Width (ex: '200px', '20em')" @input="emitChange">
    <template v-if="child && child['max-width'] && child['max-width'].length > 0" v-slot:append>
      <q-btn :icon="mdiClose" size="xs" flat dense @click="child['max-width'] = ''"/>
    </template>
  </q-input>

  <q-input filled v-model="child['max-height']" dense label="Max Height (ex: '300px', '25em')" @input="emitChange">
    <template v-if="child && child['max-height'] && child['max-height'].length > 0" v-slot:append>
      <q-btn :icon="mdiClose" size="xs" flat dense @click="child['max-height'] = ''"/>
    </template>
  </q-input>
  <q-color
        v-model="child['background-color']"
...
export default {
  ...
  watch: {
    classes (e) {
      this.$emit('inputClass', e)
    },
    styles (e) {
      this.$emit('inputStyle', e)
    }
  },
  computed: {
    child: {
    ...
    styles () {
      if (typeof this.child === 'undefined') { return '' }
      return 'overflow: auto;' +
        (this.child['max-height'] ? ('max-height: ' + this.child['max-height'] + '; ') : '') +
        (this.child['max-width'] ? ('max-width: ' + this.child['max-width'] + '; ') : '') +
        (this.child['min-height'] ? ('min-height: ' + this.child['min-height'] + '; ') : '') +
        (this.child['min-width'] ? ('min-width: ' + this.child['min-width'] + ';') : '') +
        (this.child['background-color'] ? ('background-color: ' + this.child['background-color'] + ';') : '')
    },
    classes () {
      if (typeof this.child === 'undefined') { return '' }
      return (typeof this.child.widthGroup === 'undefined' ? '' : this.child.widthGroup) +
        ' ' + (typeof this.child.breakpointGroup === 'undefined' ? '' : this.child.breakpointGroup) +
        ' ' + (typeof this.child.alignmentGroup === 'undefined' ? '' : this.child.alignmentGroup) +
        ' ' + (typeof this.child.offsetGroup === 'undefined' ? '' : this.child.offsetGroup) +
        ' ' + (typeof this.child.gutterGroup === 'undefined' ? '' : this.child.gutterGroup) +
        ' ' + (typeof this.child.colGutterGroup === 'undefined' ? '' : this.child.colGutterGroup)
        .replace(/,/g, ' ')
        .replace(/' '/g, ' ')
    }
  },
